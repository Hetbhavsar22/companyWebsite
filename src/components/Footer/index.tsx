...
import FooterLogo from "@/public.icons/YD-logo.svg";
      ...
      <div className=" text-background w-full grid md:grid-cols-[30%_1fr] grid-cols-1 md:gap-x-20 max-md:gap-y-9 md:gap-y-20">
        <div className="overflow-hidden">
          <Link title="home-link" href={"/"}>
            ...
            <Image src="/icons/YD-logo.jpg" alt='Company Logo' width="100" height="100" />
          </Link>
        </div>
        ...
        <div className="">
          ...
          <p className="text-offwhite" title="address">{data.address}</p>
        </div>
        <div className="flex flex-col max-md:justify-between gap-y-8">
          <div className="flex flex-col lg:text-subtitle1">
            <div>
              <Link title="email" href="mailto:yamunadigitechnology@gmail.com">
                {data.email}
              </Link>
            </div>
            <div>
              <Link title="mobile" href={"tel:7433850325"}>
                {data.mobile}
              </Link>
            </div>
          </div>
